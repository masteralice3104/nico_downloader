...
| RAM              | 8GB以上（推奨：16GB以上）                                            | 8GB未満はサポート対象外<br>変換前にメモリ上に保存する関係でメモリを大量に使用<br>動画1時間あたり約2-4GBのメモリ消費を想定 |
...
| 4. ダウンロードしようとしたら1KBになりました。バグらささっていると思います。                           | 十中八九変換に失敗しています。<br>原因の一例<br>・PCのメモリが足りない(要件を読んで下さい)<br>・回線の問題でダウンロードに失敗した(要件を読んで下さい)<br>・動画が暗号化されている(おことわりを読んで下さい)<br>・動画が長すぎる(特に3時間以上)<br>※v5.0.0.20でメモリ不足エラーを大幅に改善しました                                       |
...
| 5.0.0.16     | 2025.02.22 | ニコニコの画面更新に対応                                                                                                             |
| 5.0.0.17     | 2025.02.22 | オプション初期処理が抜けていたのを修正                                                                                               |
| 5.0.0.18     | 2025.02.24 | モードが取得できなかった場合はデフォルトのmp4にする仕組み導入                                                                        |
| 5.0.0.19     | 2025.07.01 | ニコニコの画面更新に対応                                                                                                             |
| 5.0.0.20     | 2025.08.20 | ニコニコの画面更新に対応<br>大量ファイル処理時のメモリ不足エラー対策<br>変換進捗表示機能追加<br>動的バッチサイズ調整による安定性向上 |